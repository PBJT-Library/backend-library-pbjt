...
const UpdateLoanBody = t.Object({
  due_date: t.Optional(t.String({ format: 'date' })),
  status: t.Optional(t.Union([t.Literal('active'), t.Literal('completed'), t.Literal('overdue')])),
  notes: t.Optional(t.String()),
  book_id: t.Optional(t.Number({ minimum: 1, description: 'Book ID to change to' })),
});
...
export const loanRoute = new Elysia({ prefix: '/loans' })
  ...
  .get(
    ...
    },
    {
      detail: {
        ...
        description: 'Mengambil seluruh data peminjaman buku beserta informasi buku dan member',
      },
    }
  )
  ...
  .get(
    ...
    },
    {
      detail: {
        ...
        description: 'Mengambil detail data peminjaman berdasarkan loan_id (e.g., LN001)',
      },
    }
  )
  ...
  .post(
    '/',
    async ({ body }) => {
      console.log('[DEBUG] POST /loans called with body:', JSON.stringify(body, null, 2));
    ...
    },
    {
      ...
      detail: {
        ...
        description: 'Membuat data peminjaman buku baru (1 buku per loan, max 3 loans per member)',
      },
    }
  )
  ...
  .put(
    '/:loan_id',
    async ({ params, body }) => {
      console.log('[DEBUG] PUT /loans/:loan_id called with:', JSON.stringify(body, null, 2));
  ...
  .patch(
    '/:loan_id/return',
    async ({ params, body }) => {
      console.log('[DEBUG] PATCH /loans/:loan_id/return called for:', params.loan_id);
      const loan = await LoanService.returnBook(params.loan_id, body || undefined);
